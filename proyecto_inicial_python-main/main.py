...
def leer_palabra_secreta(lista):
    with open(lista, "r") as csvfile:
        palabras = list(csv.DictReader(csvfile))
    secret = random.choice(palabras)
    palabra_secreta = secret["palabras"]
...
def verificar_letra(letras, palabra_secreta):
    if letras in palabra_secreta:
        return True
    else:
        return False
    ...
    while True:
        for letra_secreta in palabra_secreta:
            if letra_secreta in letras_usadas:
                ...
                print("aun no se adivino la palabra")
                return False
                break
        return True
        break
